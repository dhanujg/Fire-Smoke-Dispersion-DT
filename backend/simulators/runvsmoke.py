...
import argparse, cgi, cgitb; cgitb.enable()
import time
...
sys.path.insert(0, str(ROOT))          # ensure `import backend...` always works


# ------------------------------------------------------------------
# Use Wine path from global config
# ------------------------------------------------------------------
from backend.utils.paths import conf
WINE = conf.vsmoke["wine"]
...
import os
vsmokepath = str(ROOT/"backend"/"vsmoke_bin")
os.environ["WINEPATH"] = vsmokepath + ";" + os.environ.get("WINEPATH", "")
outputpath = str(ROOT/"data"/"plumes")

parser = argparse.ArgumentParser()
parser.add_argument('--input', default=str(ROOT/"data"/"smokeinput.json"))
args, _ = parser.parse_known_args()

# later in the file (single occurrence):
...
try:
    # run the Fortran solver – with Wine on Linux/macOS, directly on Windows
    if WINE:                                     # non‑empty → use Wine
        result = subprocess.check_output([WINE, "VSMKARC.EXE"], shell=False)
    else:                                        # empty string → call EXE directly
        result = subprocess.check_output(["VSMKARC.EXE"],  shell=False)
    print(result)
...
if __name__ == "__main__":
    with open(args.input) as fp:
        params = json.load(fp)
    # …(existing code that expects `params`)…
